...
bool Import3DFromFile(const std::string& pFile, std::map<std::string, GLuint*> &textureIdMap)
{
	...
	scene = import->ReadFile(pFile, aiProcessPreset_TargetRealtime_Quality);
...
int InitGL(std::map<std::string, GLuint*> &textureIdMap)					 // All Setup For OpenGL goes here
{
	if (!LoadGLTextures(scene, textureIdMap))
	{
		return FALSE;
	}
...
void drawAiScene(const aiScene* scene, std::map<std::string, GLuint*> &textureIdMap)
{
	logInfo("drawing objects");

	recursive_render(scene, scene->mRootNode, 0.5, textureIdMap);

}

int DrawGLScene(std::map<std::string, GLuint*> &textureIdMap)				//Here's where we do all the drawing
{
	...
	drawAiScene(scene, textureIdMap);
...
BOOL CreateGLWindow(const char* title, int width, int height, int bits, bool fullscreenflag, std::map<std::string, GLuint*> &textureIdMap)
{
...
//int main(int argc, char* argv[])
{
	...
	*/
	...
	modelpath = "D:/IMAC/OpenGLAdvanced/SweepingBirds/Objects/Birds/BirdAlone.obj";
	// images / texture
	std::map<std::string, GLuint*> textureIdMap;	// map image filenames to textureIds

	if (!Import3DFromFile(modelpath, textureIdMap)) return 0;
	...
	if (!CreateGLWindow(windowTitle, 640, 480, 16, fullscreen, textureIdMap))
	{
		return 0;
	}
